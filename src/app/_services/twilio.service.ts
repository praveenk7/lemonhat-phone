...
export class TwilioService{
    ...
    //baseURL: string = "http://192.168.2.61:8080";
   ...
   getTwilioClient(){
    ...
    sendMessage(itemId: any, msg: any, itemListId: any, userId: any, msgType:any) {
        return this.http.post(this.baseURL + '/chatconversation', { "itemId": itemId, "msg": msg, "userId": userId, "itemListId": itemListId, "msgType": msgType, "createdDate": new Date()}).map((response: Response) => {
            return response;
        });
    }

    updateBought(itemData:any,bought:any) {
        return this.http.post(this.baseURL + '/itembought', { "bought": bought, "itemId": itemData.item}).map((response: Response) => {
            return response;
        });
    }
